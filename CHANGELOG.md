## 0.24.0
* Добавлен доступ к серверному uid пользователя: getUserId(), setUserIdListener() и restoreUserId() для восстановления пользователя после переустановки приложения
* Запросы, дождавшиеся resetUser() или restoreUserId(), теперь заново выбирают владельца сессии, а не уходят параллельно без сессии
* Исправлено: запросы контента (getContent*, inline-виджеты, автопоказ in-app) теперь уходят от пользователя, заданного через setUser(); раньше это касалось только visit и event. Показы, лимиты частоты и A/B-варианты для такого контента теперь считаются для указанного пользователя, а не для анонимной сессии устройства

## 0.23.1
* Исправлена ошибка компиляции с dio 5.10.0 и новее (`DioExceptionType.transformTimeout`)

## 0.23.0
* Добавлена поддержка офлайн-доставки событий: сохранение очереди на устройстве и повторная отправка при восстановлении соединения
* Добавлены настройки OfflineQueueSettings, методы flushQueue() и clearQueue(), а также pendingDeliveries для проверки размера очереди
* Добавлены повторные попытки запросов при временных ошибках и staleContentTimeout для ограничения показа устаревшего контента
* Добавлен доступ к исходным variables через CampaignContent.rawVariables
* Добавлен ContentClickEngagement для отслеживания кликов по контенту, отображаемому приложением
* У всех событий добавлены customProps и eventTime, у CustomEvent — cuid, cuidType и cart
* Исправлено дублирование ручки перетаскивания BottomSheet при включении её в теме приложения
* Обновлён example

## 0.22.2
* Доработана поддержка in-app форм
